...
class PaymentPage(Base):
    ...
    bank_payment_button = (
        "//buton[@class='_14KXZFF2qamrgjaDJQf2-k _2LEfKwbPi7lA1km6XdQKz0']"
    )
    ...
    def get_new_card(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.new_card))
        )

    def get_continue_button(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.continue_button))
        )

    def get_card_number(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.card_number))
        )

    def get_expiry_date(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.expiry_date))
        )

    def get_card_cvc(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.card_cvc))
        )

    def get_bank_payment_button(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.bank_payment_button))
        )

    def get_payment_button(self):
        return WebDriverWait(self.driver, 30).until(
            EC.element_to_be_clickable(("xpath", self.payment_button))
        )
